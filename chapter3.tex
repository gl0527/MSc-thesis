...
%----------------------------------------------------------------------------
Ez a fejezet tartalmazza a \sectref{Intro}.~fejezetben ismertetett feladataimnak a megoldásait.

Elsõként bemutatja a játékobjektumok késleltetett létrehozásának megvalósítását, ezután az
...
Végezetül a fejezet a -- már alapjaiban eddig is meglevõ -- hangrendszer illetve részecskerendszer
továbbfejlesztésével illetve az elkészült rendszer áttekintésével zárul.

%----------------------------------------------------------------------------
\section{Játékobjektumok késleltetett létrehozása}\label{sect:Prefab}
%----------------------------------------------------------------------------
...
csak a virtuális világ egy késõbbi szakaszában lehetett volna véghezvinni.
...
Másfelõl annak is van értelme, ha többféle lövedék típus létezik egy játékban, s ezeknek vannak közös
...
A C\texttt{++} oldal mellett a jelenet XML leírása is módosult természetesen. Az eddigi funkcionalitások mellett
...
állapotra delegálja, melyet egy absztrakt õs objektumra mutató pointerrel ér el, kihasználva a C\texttt{++}
...
Ezek után már csak a C\texttt{++} nyelv polimorfizmusát kell használni, miszerint a \verb+Context+ osztály tagjaként
...
az állapotokban való tartózkodást illetve az állapotátmeneteket hogyan kezelje. Ehhez felhasználja a
\sectref{Events} szakaszban ismertetett Delegate osztályt. Az osztály mûködésének
...
%----------------------------------------------------------------------------
\section{Animációs rendszer}\label{sect:Animation}
%----------------------------------------------------------------------------
...
szokás hangokkal jelezni a felhasználó felé (pl.: fegyver felvétele, szintlépés, stb.)\cite{SoundDesign}.
...
\subsection{Megvalósítás}
%----------------------------------------------------------------------------
A megvalósításhoz az ingyenes, platformfüggetlen OpenAL-t használtam fel. Ez egy háromdimenziós hang
rendereléssel foglalkozó függvénykönyvtár, mely API szinten nagyon hasonlít az OpenGL-hez, így
a használata a grafikában jártas fejlesztõk számára könnyen elsajátítható.

\begin{figure}[!ht]
\centering
\includegraphics[width=40mm, keepaspectratio]{figures/OpenAL_logo.png}
\caption{Az OpenAL logója.}
...
Ez a megoldás egyébként analógia a grafikai motoroknál megszokott fényforrások kezelésére.

%----------------------------------------------------------------------------
\section{Plakátok és részecskerendszerek}\label{sect:Particles}
%----------------------------------------------------------------------------
...
Ha például füstöt (\figref{ParticlesSmoke} ábra), tüzet, hóesést, ködöt és egyéb olyan jelenséget szeretnénk megjeleníteni,
...
\caption{Példa füst megjelenítésére részecskerendszerek segítségével\cite{SmokeParticles}.}
\label{fig:ParticlesSmoke}
\end{figure}

A plakátok olyan kétdimenziós -- általában textúrázott -- síkdarabok, mely normálvektoraik
mindig az aktuális kamera nézeti irányával ellentétes irányúak, vagyis a kurrens nézeti síkkal
párhuzamosak. Ezzel a technikával háromdimenziós érzetet lehet kelteni a felhasználóban, hiszen
az imént leírt tulajdonsága miatt a geometria kétdimenziós mivoltát el lehet titkolni a felhasználó
elõl. A plakátokat általában növényzet illetve távoli objektumok megjelenítésére és részecskerendszerek
definiálásához használják.

Viszont önmagában egyetlen kétdimenziós síklap nem elegendõ a korábbiakban említett effektek vizuális megjelenítésére,
ellenben nagy mennyiségû használatuk segítségével igen is valósághû eredményeket lehet produkálni.
A számítógépes grafikában a részecskerendszerek éppen olyan objektumok, melyek sok-sok megfelelõen elhelyezett
és textúrázott plakát segítségével jelenítik meg a korábban említett jelenségeket.

Általában a részecskéket kibocsátó objektumot \verb+emitter+-nek nevezik. Szokás megadni a részecskék
keletkezési gyakoriságát és mennyiségét, kezdõsebességét, élettartamát, színét és még sok-sok más
paramétert az imént felsoroltakon kívül. Az is jellemzõ, hogy a fejlesztõk nem konkrét értékeket adnak
meg a paramétereknek, hanem várható értékkel és szórással vagy minimum és maximum korlátokkal
definiálják a mennyiségeket, amivel még valósághûbb eredményeket lehet elérni.
...
\section{Áttekintés}
%----------------------------------------------------------------------------
Ez a szakasz az ebben a fejezetben ismertetett általános célú, komponens alapú játékmotorom
funkcionalitásainak összefoglalását teszi meg.

Elsõként azokra a funkcionalitásokra szeretnék kitérni, melyek már az alapképzés során elkészített
rendszeremben is jelen voltak, de átdolgozásra kerültek a mesterképzésem alatt. Ilyen funkcionalitás
a fizika, melyben a \sectref{PhyMaterial} szakaszban leírtaknak megfelelõen változott az anyagok leírása, illetve
a \sectref{PhyEvents} szakaszban ismertetett módon bõvült a rendszer a fizikai események kezelésével.
Emellett a megjelenítés is egy olyan része a rendszernek, mellyel természetesen már rendelkezett a korábbi
megvalósításom is, viszont számos ponton bõvült azóta. Itt példaként említhetõ a \sectref{Animation} szakaszban
ismertetett animációs alrendszer, a \sectref{GUI} szakaszban kifejtett felhasználói felület illetve a
\sectref{Particles} szakaszban taglalt részecskerendszerek.

Ezek mellett kerültek be olyan funkcionalitások is a rendszerembe, melyek az alapképzés során elkészített
motoromnak nem voltak részei. Ilyen funkcionalitás a \sectref{Audio} szakaszban leírt hangrendszer, vagy a 
\sectref{Prefab} szakaszban ismertetett játékobjektumok késleltetett létrehozása.

Összességében elmondható, hogy az alapképzés során elkészített komponens alapú játékmotorhoz képest a jelen dolgozat
által ismertetett játékmotor egy merõben más rendszer.
